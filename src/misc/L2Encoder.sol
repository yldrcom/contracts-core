...
contract L2Encoder {
    ...
    /**
     * @notice Encodes repay with yToken parameters from standard input to compact representation of 1 bytes32
     * @param asset The address of the borrowed underlying asset previously borrowed
     * @param amount The amount to repay
     * - Send the value type(uint256).max in order to repay the whole debt for `asset` on the specific `debtMode`
     * @param interestRateMode The interest rate mode at of the debt the user wants to repay: 1 for Stable, 2 for Variable
     * @return compact representation of repay with yToken parameters
     */
    function encodeRepayWithYTokensParams(address asset, uint256 amount, uint256 interestRateMode)
        external
        view
        returns (bytes32)
    {
    ...
    function encodeLiquidationCall(
        ...
        assembly {
            ...
            res2 := add(shortenedDebtToCover, shl(128, receiveYToken))
        }
        return (res1, res2);
    }
}
