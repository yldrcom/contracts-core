...
contract L2Pool is Pool, IL2Pool {
    ...
    /// @inheritdoc IL2Pool
    function repayWithYTokens(bytes32 args) external override returns (uint256) {
        (address asset, uint256 amount, uint256 interestRateMode) = CalldataLogic.decodeRepayParams(_reservesList, args);

        return repayWithYTokens(asset, amount, interestRateMode);
    }
    ...
    function liquidationCall(bytes32 args1, bytes32 args2) external override {
        (address collateralAsset, address debtAsset, address user, uint256 debtToCover, bool receiveYToken) =
            CalldataLogic.decodeLiquidationCallParams(_reservesList, args1, args2);
        liquidationCall(collateralAsset, debtAsset, user, debtToCover, receiveYToken);
    }
}
